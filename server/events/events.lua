RegisterNetEvent('rsg-inventory:server:closeInventory', function(inventory)
    ...
    -- Update both items and last_accessed time for stash cleanup tracking
    MySQL.prepare('INSERT INTO inventories (identifier, items, last_accessed) VALUES (?, ?, CURRENT_TIMESTAMP) ON DUPLICATE KEY UPDATE items = ?, last_accessed = CURRENT_TIMESTAMP', { inventory, json.encode(Inventories[inventory].items), json.encode(Inventories[inventory].items) })
end)
