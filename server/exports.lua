...
Inventory.CanAddItem = function(source, item, amount)
    ...
    for _, v in pairs(Player.PlayerData.items) do
        if v then
            slotsUsed = slotsUsed + 1
            if not itemData.unique and v.name == item and v.amount then
                canStack = true
            end
        end
    end

    if slotsUsed >= Config.MaxSlots and not canStack then
        return false, 'slots'
    end
    return true
end
...
--- Helper: checks CanAddItem and (optionally) notifies the player if it fails.
--- @param source number
--- @param item string
--- @param amount number
--- @return boolean ok, string? reason
Inventory.RequireCanAddItem = function(source, item, amount)
    local canCarry, reason = Inventory.CanAddItem(source, item, amount)
    if canCarry then
        return true, nil
    end
    ...
    -- Use whatever global notification you prefer.
...
exports('RequireCanAddItem', Inventory.RequireCanAddItem)
...
Inventory.OpenInventory = function (source, identifier, data)
    ...
    -- Update last_accessed time for stash cleanup tracking
    MySQL.prepare('UPDATE inventories SET last_accessed = CURRENT_TIMESTAMP WHERE identifier = ?', { identifier })
